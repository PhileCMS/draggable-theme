...
;( function( window ) {
	...
	DragSlideshow.prototype._toggleContent = function( slide ) {
		...
		if( this.isContent ) {
			...
			this.dd.enable();
			this.el.classList.remove( 'show-content' );
		}
		else {
	...
	DragSlideshow.prototype._navigate = function( x, y ) {
		...
		this.slides[ this.current || 0 ].classList.remove( 'current' );
	...
	DragSlideshow.prototype.toggle = function() {
		...
		this.el.classList.remove( this.isFullscreen ? 'switch-max' : 'switch-min' );
		this.el.classList.add( this.isFullscreen ? 'switch-min' : 'switch-max' );
		...
		var onEndTransitionFn = function( ev ) {
			...
			this.classList.remove( self.isFullscreen ? 'img-dragger-large' : 'img-dragger-small' );
			this.classList.add( self.isFullscreen ? 'img-dragger-small' : 'img-dragger-large' );
